...
def _batch_response_format(max_items: int) -> dict[str, Any]:
    ...
    # Mở rộng primary_type để tránh lỗi parse
    if "primary_type" in properties:
        properties["primary_type"] = {
            "type": "string",
            "enum": [
                "Product", "Society & Culture", "Practical",
                "Infrastructure", "Business", "Technology",
                "Research", "Policy", "Security", "Enterprise", "AI Tools"
            ]
        }
...
def _batch_system_prompt() -> str:
    return (
        ...
        "Bạn xử lý nhiều bài viết ĐỘC LẬP. Không so sánh chéo giữa các item.\n"
        "primary_type chỉ được dùng các giá trị sau: Product, Society & Culture, Practical, Infrastructure, Business, Technology, Research, Policy, Security, Enterprise, AI Tools.\n"
        "Trả về đúng JSON với key `articles`. Mỗi item phải có `item_id` và đầy đủ field.\n"
        "Tuân thủ nghiêm ngặt schema. Không thêm text nào ngoài JSON."
    )
