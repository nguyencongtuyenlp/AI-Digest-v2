...
class MVP3SpeedOptimizedTest(unittest.TestCase):
    ...
    @patch("nodes.batch_classify_and_score_node.write_temporal_snapshot", return_value="reports/mock.json")
    @patch(
        "nodes.batch_classify_and_score_node.run_json_inference_meta",
        return_value=(
            {
                "articles": [
                    {
    ...
    @patch(
        "nodes.batch_deep_process_node.run_json_inference_meta",
        return_value=(
            {
                "articles": [
                    {
    ...
    @patch("nodes.batch_deep_process_node._search_community_reactions", return_value="Chưa có dữ liệu cộng đồng")
