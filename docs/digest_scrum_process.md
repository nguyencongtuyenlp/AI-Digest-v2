...
- dựng brief Telegram theo 3 lane
...
- lane nào cần chú trọng hơn: `main`, `GitHub`, `Facebook`
...
flowchart TB
  ...
  subgraph Execution["Sprint Execution - 1 batch digest"]
    ...
    E5["Classify + score + chia top/low"]
    E6["Deep analysis cho bài top"]
    E7["Compose short note + delivery judge"]
    E8["Save Notion + summarize + quality gate"]
  end

  subgraph Review["Review / Approve"]
    R1["UI preview 3 lane\nMain / GitHub / Facebook"]
...
Hệ thống đang có health model deterministic trong `run_health.py` để trả lời:
...
- `Classify + score`: chấm bài theo góc nhìn editorial/operator
- `Deep analysis`: chỉ chạy cho bài điểm cao
- `Compose`: nén thành short note dùng cho delivery
- `Delivery judge`: quyết định bài nào lên `main`, bài nào vào lane phụ, bài nào bỏ
...
- GitHub lane và Facebook lane có ý nghĩa không
- summary có warning không
